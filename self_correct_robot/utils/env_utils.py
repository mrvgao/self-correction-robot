...
from self_correct_robot.utils.log_utils import log_warning
...
def get_env_class(env_meta=None, env_type=None, env=None):
    ...
    if env_type == EB.EnvType.ROBOSUITE_TYPE:
        from self_correct_robot.envs.env_robosuite import EnvRobosuite
        return EnvRobosuite
    elif env_type == EB.EnvType.GYM_TYPE:
        from self_correct_robot.envs.env_gym import EnvGym
        return EnvGym
    elif env_type == EB.EnvType.IG_MOMART_TYPE:
        from self_correct_robot.envs.env_ig_momart import EnvGibsonMOMART
...
def wrap_env_from_config(env, config):
    ...
    if config.train.frame_stack > 1:
        from self_correct_robot.envs.wrappers import FrameStackWrapper
